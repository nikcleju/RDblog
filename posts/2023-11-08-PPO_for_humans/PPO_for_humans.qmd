...
1. During rollout, collect the cumulative reward (i.e. value) out of every state, to act like a ground truth
   ...
   - Record all the tuples `(state, action taken, action probability, return (cumulative reward)`
2. Compute the estimated value of the same states with the Value Net
3. Compare and update:
   - look back on all the `(state, action taken, action probability, cumulative reward)` tuples collected, and:
   - compute the action **advantages** = difference between the return when taking that action - the state value (averaging all actions from that state)
   - for actions with positive advantage, increase the probability of those actions (they are better than the average action)
   - for actions with negative advantage, decrease the probability of those actions (they are worse than the average action)

**To rethink**: The equilibrium state, when both Action Net and Value Net are fully converged, is
