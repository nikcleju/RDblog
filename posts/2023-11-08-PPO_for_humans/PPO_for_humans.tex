...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{img/ActionNet.png}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{img/ValueNet.png}}
...
The general idea of PPO learning algorithm is:
...
\item
  ...
  \item
    ...
    \texttt{(state,\ action\ taken,\ action\ probability,\ return\ (cumulative\ reward)}
  \end{itemize}
\item
...
\item
  ...
  \item
    ...
    \texttt{(state,\ action\ taken,\ action\ probability,\ cumulative\ reward)}
    tuples collected, and:
  \item
    compute the action \textbf{advantages} = difference between the
    return when taking that action - the state value (averaging all
    actions from that state)
  \item
    for actions with positive advantage, increase the probability of
    those actions (they are better than the average action)
  \item
    for actions with negative advantage, decrease the probability of
    those actions (they are worse than the average action)
  \end{itemize}
\end{enumerate}

\textbf{To rethink}: The equilibrium state, when both Action Net and
Value Net are fully converged, is when the cumulative rewards collected
in the rollout stage, with the current Action Net (current policy), are
identical to those states' values as computed by the Value Net.
...
\pandocbounded{\includegraphics[keepaspectratio]{img/Explanation.png}}
