...
This was implemented by multiplying with $\alpha$ in the inner problem alongside $\omega_i x^i \lambda$, as a consequence of $\alpha G$ with $G$ defined as in (@eq-FG):
...
but just the constraint. This makes the Lagrangean steeper
w.r.t. to $\lambda$ variables, which impacts the convergence speed.
Basically, we just increase step size towards the same solution.
Thus, we can get instability.
...
1. For the OC-SVM, we can't just apply standard OC-SVM.
   ...
   of the extra $\sqrt{\alpha}$ factor in the constraint.
